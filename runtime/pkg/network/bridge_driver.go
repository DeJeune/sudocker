...
func (d *BridgeNetworkDriver) Create(subnet string, name string) (*config.Network, error) {
	ip, ipRange, _ := net.ParseCIDR(subnet)
	ipRange.IP = ip
	n := &config.Network{
		Name:    name,
		Address: ipRange.String(),
		Bridge:  d.Name(),
		SubNet:  ipRange,
	}
	// 配置Linux Bridge
...
// Delete 删除网络
func (d *BridgeNetworkDriver) Delete(network *config.Network) error {
	// 清除路由规则
	err := deleteIPRoute(network.Name, network.Address)
	if err != nil {
		return errors.WithMessagef(err, "clean route rule failed after bridge [%s] deleted", network.Name)
	}
	// 清除 iptables 规则
	err = deleteIPTables(network.Name, network.Address)
	if err != nil {
...
// Connect 连接一个网络和网络端点
// 内部会修改 endpoint.Device
func (d *BridgeNetworkDriver) Connect(networkName string, endpoint *config.Endpoint) error {
...
*/
func (d *BridgeNetworkDriver) initBridge(n *config.Network) error {
	bridgeName := n.Name
	// 1）创建 Bridge 虚拟设备
	...
	if err := setInterfaceIP(bridgeName, gatewayIP); err != nil {
		return errors.Wrapf(err, "Error set bridge ip: %s on bridge: %s", gatewayIP, bridgeName)
	}
	// 3）启动 Bridge 设备
	...
	// 4）设置 iptables SNAT 规则
	if err := setupIPTables(bridgeName, n.Address); err != nil {
...
func (d *BridgeNetworkDriver) deleteBridge(n *config.Network) error {
	bridgeName := n.Name
...
func deleteIPTables(bridgeName string, subnet string) error {
	return configIPTables(bridgeName, subnet, true)
}

func configIPTables(bridgeName string, subnet string, isDelete bool) error {
	action := "-A"
	if isDelete {
		action = "-D"
	}
	// 拼接命令
	iptablesCmd := fmt.Sprintf("-t nat %s POSTROUTING -s %s ! -o %s -j MASQUERADE", action, subnet, bridgeName)
	cmd := exec.Command("iptables", strings.Split(iptablesCmd, " ")...)
	logrus.Infof("配置 SNAT cmd: %v", cmd.String())
	// 执行该命令
	output, err := cmd.Output()
	if err != nil {
		return errors.Errorf("iptables Output, %v", output)
	}
	return nil
}
