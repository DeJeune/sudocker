...
const ipamDefaultAllocatorPath = "/var/lib/sudocker/network/ipam/subnet.json"
...
// 初始化一个IPAM的对象，默认使用/var/lib/sudocker/network/ipam/subnet.json作为分配信息存储位置
...
func (ipam *IPAM) Allocate(subnet *net.IPNet) (ip net.IP, err error) {
	...
	if _, exist := (*ipam.Subnets)[subnet.String()]; !exist {
		...
		(*ipam.Subnets)[subnet.String()] = strings.Repeat("0", 1<<uint8(size-bit))
	}
	// 遍历网段的位图数组
	...
	err = ipam.dump()
	if err != nil {
		err = errors.Errorf("Allocate: dump ipam error %v", err)
	}
	return
}
...
func (ipam *IPAM) Release(subnet *net.IPNet, ipaddr *net.IP) error {
	...
	err = ipam.dump()
	if err != nil {
		return errors.Errorf("Allocate: dump ipam error %v", err)
	}
	return nil
}
...
func (ipam *IPAM) dump() error {
	...
	if _, err := os.Stat(ipamConfigFileDir); err != nil {
		...
		if err = os.MkdirAll(ipamConfigFileDir, 0o644); err != nil {
			return err
		}
	}
	// 打开存储文件 O_TRUNC 表示如果存在则消空， os O_CREATE 表示如果不存在则创建
	subnetConfigFile, err := os.OpenFile(ipam.SubnetAllocatorPath, os.O_TRUNC|os.O_WRONLY|os.O_CREATE, 0o644)
	if err != nil {
		return err
	}
	defer subnetConfigFile.Close()
	encoder := json.NewEncoder(subnetConfigFile)

	return encoder.Encode(ipam.Subnets)
}
